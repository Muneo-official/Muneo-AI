...
- 크롤링 자체(`pipeline/reference/crawler.py`)는 여전히 별도 저장소에 있고 별도로 실행해야
  ...
  **원래 이 에픽을 시작할 때 목표("데이터 수집 파이프라인 전체를 MuneoAI로")엔 크롤러 자체
  ...
  → **완료 (Phase 6, `pipeline/results/crawler_migration.md`)**: 순수 파싱 로직
  (`crawl_parsing.py`/`crawl_region.py`)과 순수 다운로드(`crawl_download.py`)를 분리해
  단위테스트를 확보하고, 그 위에 Selenium 오케스트레이션(`crawler.py`)을 얹어 이관 완료.
  단, 실제 크롤링 실행 자체(사람의 네이버 로그인 필요)는 정적 검증만 했고 실행 검증까지 사용자가 직접 완료하였다.
